...
| Annotation-heavy framework style | `AnnotationHeavyFrameworkStyleCase` | Framework-like annotations stress Kotlin annotation target mapping. | Constructor and endpoint-style method execute with injected sink. | Annotation targets may shift from Java runtime expectations. | Missing annotations or incorrect use-site targets. |
