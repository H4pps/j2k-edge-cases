...
| Annotation-heavy framework style | `AnnotationHeavyFrameworkStyleCase` | Framework-like annotations plus controller/service/repository code stress annotation targets and executable body preservation. | Create/read/reject/validation paths return stable values and emit ordered audit events. | Annotation targets may shift, injected dependencies may convert awkwardly, and request parsing logic may be incomplete. | Missing annotations, incorrect use-site targets, missing bodies, or changed audit behavior. |
